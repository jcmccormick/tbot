...
func getTokenBody(data url.Values) {
  ...
  if err := json.Unmarshal(body, &cred); err != nil { log.Fatal(err) }
...
func getUserId(user string) (string) {
  ...
  if err := json.Unmarshal(body, &userRequest); err != nil { log.Fatal(err) }
...
func getRewards(userId string) ([]Reward) {
  ...
  if err := json.Unmarshal(body, &rewardRequest); err != nil { log.Fatal(err) }
